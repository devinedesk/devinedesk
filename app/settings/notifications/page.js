...
export default async function NotificationsSettings() {
  ...
  const setting = await prisma.setting.findUnique({
    where: { userId_key: { userId: session.user.id, key: 'notifications_email_marketing' } },
  });
  ...
  return (
    <div className="space-y-6 max-w-4xl animate-in fade-in slide-in-from-bottom-4 duration-500">
      <div>
        <h2 className="text-2xl font-bold tracking-tight text-white capitalize">Notifications</h2>
        ...
        <form
          action={async (formData) => {
            ...
            await prisma.setting.upsert({
              where: {
                userId_key: { userId: session.user.id, key: 'notifications_email_marketing' },
              },
              update: { value: val },
              create: { userId: session.user.id, key: 'notifications_email_marketing', value: val },
            });
          }}
        >
          <div className="flex items-center justify-between">
            <div>
              <h3 className="font-medium text-white capitalize">Marketing Emails</h3>
              <p className="text-sm text-neutral-400 mt-1">
                Receive product updates, newsletters, and promotional offers.
              </p>
            </div>
