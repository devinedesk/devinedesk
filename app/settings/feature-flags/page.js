...
export default async function FeatureFlagsSettings() {
  ...
  const setting = await prisma.setting.findUnique({
    where: { userId_key: { userId: session.user.id, key: 'feature-flags_enabled' } },
  });
  ...
  return (
    <div className="space-y-6 max-w-4xl animate-in fade-in slide-in-from-bottom-4 duration-500">
      <div>
        <h2 className="text-2xl font-bold tracking-tight text-white capitalize">Feature Flags</h2>
        <p className="text-neutral-secondary mt-1">Manage your feature flags preferences.</p>
      </div>
        ...
        <form
          action={async (formData) => {
            ...
            await prisma.setting.upsert({
              where: { userId_key: { userId: session.user.id, key: 'feature-flags_enabled' } },
              update: { value: val },
              create: { userId: session.user.id, key: 'feature-flags_enabled', value: val },
            });
          }}
        >
          <div className="flex items-center justify-between">
            <div>
              <h3 className="font-medium text-white capitalize">Enable Feature Flags</h3>
