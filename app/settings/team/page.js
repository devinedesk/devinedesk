...
export default async function TeamSettings() {
  ...
  const setting = await prisma.setting.findUnique({
    where: { userId_key: { userId: session.user.id, key: 'team_sync_enabled' } },
  });
  ...
  return (
    <div className="space-y-6 max-w-4xl animate-in fade-in slide-in-from-bottom-4 duration-500">
      <div>
        <h2 className="text-2xl font-bold tracking-tight text-white capitalize">Team</h2>
        <p className="text-neutral-secondary mt-1">
          Manage team synchronization and cross-workspace visibility.
        </p>
      </div>
        ...
        <form
          action={async (formData) => {
            ...
            await prisma.setting.upsert({
              where: { userId_key: { userId: session.user.id, key: 'team_sync_enabled' } },
              update: { value: val },
              create: { userId: session.user.id, key: 'team_sync_enabled', value: val },
            });
          }}
        >
          <div className="flex items-center justify-between">
            <div>
              <h3 className="font-medium text-white capitalize">Auto-sync Team Profiles</h3>
              <p className="text-sm text-neutral-400 mt-1">
                Keep team avatars and titles in sync across the organization.
              </p>
            </div>
