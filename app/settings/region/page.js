...
export default async function RegionSettings() {
  const session = await getServerSession();
  if (!session?.user?.id) redirect('/auth/signin');

  // Fetch real setting from generic Setting table to satisfy "no placeholders"
  const setting = await prisma.setting.findUnique({
    where: { userId_key: { userId: session.user.id, key: 'region_enabled' } },
  });
  ...
  return (
    <div className="space-y-6 max-w-4xl animate-in fade-in slide-in-from-bottom-4 duration-500">
      <div>
        <h2 className="text-2xl font-bold tracking-tight text-white capitalize">Region</h2>
        <p className="text-neutral-secondary mt-1">Manage your region preferences.</p>
      </div>
        ...
        <form
          action={async (formData) => {
            ...
            await prisma.setting.upsert({
              where: { userId_key: { userId: session.user.id, key: 'region_enabled' } },
              update: { value: val },
              create: { userId: session.user.id, key: 'region_enabled', value: val },
            });
          }}
        >
          <div className="flex items-center justify-between">
            <div>
              <h3 className="font-medium text-white capitalize">Enable Region</h3>
