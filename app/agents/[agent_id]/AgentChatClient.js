      ...
      <div className="flex-none p-6 border-b border-muted/30 bg-panel-bg">
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-sm text-secondary">{agentDetails?.description || "Ready to assist you."}</p>
          </div>
        </div>
      </div>
      ...
      <div className="flex-1 overflow-y-auto p-6 space-y-6">
        {messages.length === 0 ? (
          <div className="flex flex-col items-center justify-center h-full text-center space-y-4">
            ...
            <p className="text-secondary max-w-sm">Send a message to begin chatting with {agentDetails?.name || "the agent"}.</p>
          </div>
        ) : (
