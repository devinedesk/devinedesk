...
export const POST = withApiAuth({
    handler: async (request, { auth, body, params }) => {
        ...
        if (segments[0] === 'create') {
            ...
            return NextResponse.json({
                ...newWf,
                nodes: JSON.parse(newWf.nodes || '[]'),
                edges: JSON.parse(newWf.edges || '[]'),
                viewport: newWf.viewport ? JSON.parse(newWf.viewport) : { x: 0, y: 0, zoom: 1 }
            });
        }
        ...
        if (segments.length === 2 && segments[1] === 'api-execute') {
            ...
            try {
                const { run, jobId } = await WorkflowService.queueWorkflowRun(segments[0], auth.user.id, parsed.data.inputs);
                return NextResponse.json({ run_id: run.id, status: "processing", jobId });
            } catch (error) {
                return NextResponse.json({ detail: error.message }, { status: error.message === 'Workflow not found' ? 404 : 500 });
            }
        }
