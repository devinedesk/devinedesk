...
import { deleteObjects, downloadObject, uploadBuffer } from "../lib/storage.js";
...
adminTemplatesRouter.delete("/:id", requireAdmin, async (req: AuthedRequest, res) => {
  const template = await ownedTemplate(req, res);
  if (!template) return;
  // Gather all object keys before deleting the DB rows (cascade won't clean MinIO).
  const [blocks, audioClips] = await Promise.all([
    prisma.templateBlock.findMany({ where: { templateId: template.id } }),
    prisma.templateAudioClip.findMany({ where: { templateId: template.id } }),
  ]);
  await prisma.template.delete({ where: { id: template.id } });
  // Best-effort cleanup of stored objects.
  await deleteObjects([
    template.previewVideoKey,
    template.thumbnailKey,
    ...blocks.flatMap((b) => [
      b.startImageKey, b.endImageKey, b.swappedStartKey, b.swappedEndKey,
      b.videoKey, b.sourceVideoKey,
    ]),
    ...audioClips.map((a) => a.audioKey),
  ]);
  res.status(204).end();
});
...
adminTemplatesRouter.delete(
  ...
  async (req: AuthedRequest, res) => {
    ...
    // Clean up the block's stored objects.
    await deleteObjects([
      existing.startImageKey, existing.endImageKey,
      existing.swappedStartKey, existing.swappedEndKey,
      existing.videoKey, existing.sourceVideoKey,
    ]);
    res.status(204).end();
  },
);
...
adminTemplatesRouter.delete(
  ...
  async (req: AuthedRequest, res) => {
    ...
    // Clean up the stored audio file.
    await deleteObjects([existing.audioKey]);
    res.status(204).end();
  },
);
