...
export function BillingPage() {
  ...
  // Handle the return URL from Dodo Payments: ?payment_row=xxx
  // Try to confirm the payment (the webhook may have already granted credits).
  useEffect(() => {
    const paymentRow = searchParams.get("payment_row");
    if (!paymentRow || !session?.user) return;
    setNotice("Confirming your payment…");
    confirmPayment(paymentRow)
      .then(() => {
        setNotice("Payment confirmed! Your credits have been updated.");
        refreshCredits();
        load();
      })
      .catch((err) => {
  ...
  }, [searchParams, session?.user, load, setSearchParams]);

  const buy = useCallback(
    async (packId: string) => {
      if (!packs?.dodoConfigured) return;
      setBuying(packId);
      setError(null);
      setNotice(null);
      try {
        const session = await startCheckout(packId);
  ...
  return (
    <div className="mx-auto max-w-6xl space-y-8 px-4 py-8 sm:py-10">
      ...
      {packs && !packs.dodoConfigured && (
        <div className="rounded-xl border border-amber-500/30 bg-amber-500/10 px-4 py-3 text-sm text-amber-200">
          Payments are not configured on this server. Set <code>DODO_PAYMENTS_API_KEY</code> and{" "}
          <code>DODO_PRODUCT_IDS</code> in the backend environment to enable purchases.
        </div>
      )}
          ...
          >
            ...
            <CardContent className="space-y-4">
              ...
              <Button
                ...
                disabled={!packs.dodoConfigured || buying !== null}
                onClick={() => buy(pack.id)}
              >
                {buying === pack.id ? (
                  <>
      ...
      <Card className="border-red-500/20">
        ...
        <CardContent>
          <p className="text-sm text-muted-foreground">
            Permanently delete your account and all associated data — videos, images, avatars,
            templates, and credit history. This action cannot be undone.
          </p>
          <Button
            variant="outline"
