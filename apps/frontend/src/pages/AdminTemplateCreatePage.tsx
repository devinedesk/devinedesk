...
export function AdminTemplateCreatePage() {
  ...
  useEffect(() => {
    ...
    fetchAdminTemplates().then(setTemplates).catch((e) => console.error("Failed to load templates:", e));
    fetchModels().then(setModels).catch((e) => console.error("Failed to load models:", e));
    fetchSwapModels().then(setSwapModels).catch((e) => console.error("Failed to load swap models:", e));
    fetchAvatars().then(setAvatars).catch((e) => console.error("Failed to load avatars:", e));
  }, [session?.user]);
  ...
  async function handleCommitAudio(id: string, patch: BlockPatch) {
    ...
    try {
      ...
      setExportError(err instanceof Error ? err.message : "Failed to save audio clip");
    }
  }
  ...
  async function handleCommitBlock(id: string, patch: BlockPatch) {
    ...
    try {
      ...
      setExportError(err instanceof Error ? err.message : "Failed to save block");
    }
  }
  ...
  async function handleDeleteBlock(id: string) {
    ...
    setBlocks((prev) => prev.filter((b) => b.id !== id));
    setSelectedId((cur) => (cur === id ? null : cur));
    await deleteBlockApi(current.id, id).catch((e) => console.error("Failed to delete block:", e));
  }
