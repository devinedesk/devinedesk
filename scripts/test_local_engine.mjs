...
async function testAgent() {
    ...
    try {
        ...
        const agentRes = await fetch(`${API_BASE}/agents/create`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ name: "agent_test" }) }); const agent = await agentRes.json(); const res = await fetch(`${API_BASE}/agents/by-slug/${agent.slug}/chat`, {
            method: 'POST',
...
async function testWorkflow() {
    ...
    try {
        ...
        const edges = [
            { source: 'text_1', target: 'image_1', targetHandle: 'textInput' }
        ];
