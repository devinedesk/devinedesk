 ...
 *
 * Set LOCAL_API_KEY env var to run the live API smoke test:
 *   LOCAL_API_KEY=your_key node scripts/test_minimax_provider.mjs
 */
...
const modelsContent = readFileSync(join(ROOT, 'packages', 'studio', 'src', 'models.js'), 'utf-8');
...
if (minimaxModel.family !== 'minimax') {
  console.error(`FAIL: expected family "minimax", got "${minimaxModel.family}"`);
  process.exit(1);
}
...
if (!minimaxModel.inputs.aspect_ratio?.enum?.includes('1:1')) {
  console.error("FAIL: minimax-image-01 aspect_ratio enum does not include '1:1'");
  process.exit(1);
}
...
console.log(`      aspect ratios: ${minimaxModel.inputs.aspect_ratio.enum.join(', ')}`);
...
if (!dumpEntry) {
  console.error('FAIL: "minimax-image-01" not found in models_dump.json t2i section');
  process.exit(1);
}
...
if (!apiKey) {
  console.log('\nINFO: Skipping live API test (set LOCAL_API_KEY env var to enable).');
  console.log('\nAll checks passed.');
  process.exit(0);
}

console.log('\nRunning live API smoke test against Local API …');

const LOCAL_API_BASE = 'https://api.localapi.ai';

async function testMiniMaxImageGeneration() {
  const endpoint = minimaxModel.endpoint;
  const url = `${LOCAL_API_BASE}/api/v1/${endpoint}`;
  ...
  const pollUrl = `${LOCAL_API_BASE}/api/v1/predictions/${requestId}/result`;
  for (let i = 0; i < 30; i++) {
