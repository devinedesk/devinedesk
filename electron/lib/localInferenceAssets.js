...
function pickBinaryAssetForPlatform({ platform, arch, zipNames }) {
  ...
  if (platform === 'win32') {
    const winCandidates = candidates.filter((name) =>
      /win-(avx2?|avx512|noavx|cuda12|cu12)-x64/.test(name)
    );
    const order = [
      'win-avx2-x64',
      'win-avx-x64',
  ...
  if (platform === 'linux' && arch === 'arm64') {
    const linuxArmCandidates = candidates.filter(
      (name) => name.includes('Linux') && (name.includes('aarch64') || name.includes('arm64'))
    );
    const plain = linuxArmCandidates.find(
      (name) => !name.includes('rocm') && !name.includes('vulkan')
    );
    return (
      plain ||
  ...
  const linuxCandidates = candidates.filter(
    (name) => name.includes('Linux') && name.includes('x86_64')
  );
