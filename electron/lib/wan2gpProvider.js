...
function resolveFnNames(apiNames) {
  ...
  for (const m of WAN2GP_CATALOG) {
    ...
    if (!hit && m.family) {
      ...
      const fuzzy =
        apiNames.find((n) => n.toLowerCase().includes(m.family) && typeHint.test(n)) ||
...
async function probe(url) {
  ...
  try {
    const res = await httpJson(`${base}/config`, { timeoutMs: 5000 });
    if (res.status !== 200)
      return { ok: false, error: `HTTP ${res.status} from /config — is this a Gradio server?` };
    const cfg = JSON.parse(res.body);
...
async function uploadFile({ name, type, bytes }) {
  ...
  if (!url)
    throw new Error('Wan2GP server URL not set. Open Settings → Local Models to configure.');
  const base = normalizeUrl(url);
...
async function listModels() {
  ...
  if (!url)
    return WAN2GP_CATALOG.map((m) => ({
      ...m,
      ready: false,
      unavailableReason: 'Wan2GP URL not set',
    }));
  const base = normalizeUrl(url);
...
async function generate(params, mainWindow) {
  ...
  if (!cached) {
    await probe(url);
    cached = fnResolutionCache.get(base);
  }
  ...
  try {
    ...
    if (!mediaUrl)
      throw new Error(
        `Wan2GP returned unrecognized output: ${JSON.stringify(result).slice(0, 200)}`
      );
...
function register() {
  ...
  ipcMain.handle('wan2gp:set-url', (_, url) => {
    writeConfig({ url: normalizeUrl(url) });
    return { ok: true };
  });
