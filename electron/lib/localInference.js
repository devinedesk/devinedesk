...
const { LOCAL_AI_DIR_ENV, resolveLocalAiPaths } = require('./localInferencePaths');
...
function fetchJson(url) {
  return new Promise((resolve, reject) => {
    https
      .get(url, { headers: { 'User-Agent': 'devinedesk' } }, (res) => {
...
function downloadFile(url, destPath, onProgress) {
  ...
  const attempt = (requestUrl, redirectsLeft, retriesLeft) =>
    new Promise((resolve, reject) => {
      ...
      const reqHeaders = {
        'User-Agent': 'Mozilla/5.0 (compatible; devinedesk/1.0)',
        Accept: '*/*',
...
const CUSTOM_BINARIES = {
  'darwin-arm64':
    'https://github.com/devinedesk/devinedesk/releases/download/v1.0.3-binaries/sd-cli-metal-macos-arm64.zip',
};

async function downloadBinary(mainWindow) {
  const send = (data) =>
    mainWindow?.webContents.send('local-ai:download-progress', { id: '__binary__', ...data });
  ...
  try {
    ...
    if (customUrl) {
      ...
      if (!chosen) {
        if (process.platform === 'darwin' && process.arch !== 'arm64') {
          throw new Error(
            'Local inference on macOS only supports Apple Silicon (M1/M2/M3/M4). Mac Intel is not supported by stable-diffusion.cpp upstream.'
          );
        }
        if (process.platform === 'linux' && process.arch === 'arm64') {
          throw new Error(
            'No upstream stable-diffusion.cpp binary found for linux-arm64. Install a build that bundles local-ai/linux-arm64/bin or provide the binary manually.'
          );
        }
        const available = lastSeen.join(', ') || '(none)';
        throw new Error(
          `No binary found for ${process.platform}-${process.arch} in the last 15 releases. Latest release assets: ${available}`
        );
      }
    ...
    if (!foundBinary)
      throw new Error(`Extracted archive but could not find "${BINARY_NAME}" inside ${BIN_DIR}`);
...
async function downloadModel(modelId, mainWindow) {
  ...
  const send = (data) =>
    mainWindow?.webContents.send('local-ai:download-progress', { id: modelId, ...data });
  send({ phase: 'downloading', progress: 0 });
  ...
  try {
    ...
    throw new Error(
      `Failed to download "${model.name}" (id: ${model.id}, url: ${model.downloadUrl}): ${err.message}`
    );
  }
...
async function downloadAuxiliary(auxKey, mainWindow) {
  ...
  const id = aux.id;
  const send = (data) =>
    mainWindow?.webContents.send('local-ai:download-progress', { id, ...data });
  send({ phase: 'downloading', progress: 0 });
  ...
  try {
    ...
    throw new Error(
      `Failed to download "${aux.displayName}" (id: ${aux.id}, url: ${aux.downloadUrl}): ${err.message}`
    );
  }
...
async function generate(params, mainWindow) {
  ...
  if (!fs.existsSync(BINARY_PATH))
    throw new Error('sd.cpp binary not installed. Download it in Settings > Local Models.');
  ...
  if (model.requiresAuxiliary) {
    ...
    if (!fs.existsSync(llmPath))
      throw new Error(
        'Text encoder (Qwen3-4B) not downloaded. Go to Settings > Local Models and download all required files for Z-Image.'
      );
    if (!fs.existsSync(vaePath))
      throw new Error(
        'VAE (ae.safetensors) not downloaded. Go to Settings > Local Models and download all required files for Z-Image.'
      );
  }

  const [width, height] = arToDimensions(params.aspect_ratio || '1:1', model.type);
  const seed =
    params.seed && params.seed !== -1 ? params.seed : Math.floor(Math.random() * 2147483647);
...
function register() {
  ...
  ipcMain.handle('local-ai:download-model', (_, modelId) =>
    downloadModel(modelId, getMainWindow())
  );
  ipcMain.handle('local-ai:download-auxiliary', (_, auxKey) =>
    downloadAuxiliary(auxKey, getMainWindow())
  );
