...
export const LOCAL_MODEL_CATALOG = [
  // ── sd.cpp: Z-Image (Tongyi-MAI) ────────────────────────────────────────
  {
    ...
    name: 'Z-Image Turbo',
    description:
      "WaveSpeed's featured local model — 6B params, ultra-fast 8-step generation. No API key needed.",
    type: 'z-image',
    provider: 'sdcpp',
  ...
  },
  {
    ...
    name: 'Z-Image Base',
    description:
      'Full-quality 6B parameter model from Tongyi-MAI — higher detail, 50-step generation.',
    type: 'z-image',
    provider: 'sdcpp',
...
export const localT2VModels = LOCAL_MODEL_CATALOG.filter(
  (m) => m.provider === 'wan2gp' && m.type === 'video' && !m.needsImage
);
export const localI2VModels = LOCAL_MODEL_CATALOG.filter(
  (m) => m.provider === 'wan2gp' && m.type === 'video' && m.needsImage
);
