...
export class BillingService {
    ...
    static async finalizeSuccessfulGeneration(userId, action, prompt, model, parameters, resultUrl) {
        ...
        if (action !== 'execute-workflow') {
            recordsToCreate.push(prisma.generation.create({
                data: {
                    userId,
                    type: action,
                    prompt: prompt || '',
    ...
    static async refundFailedGeneration(userId, cost, action, prompt, model, parameters) {
        ...
        if (action !== 'execute-workflow') {
            recordsToCreate.push(prisma.user.update({
                where: { id: userId },
                data: { credits: { increment: cost } }
            }));
            recordsToCreate.push(prisma.transaction.create({
                data: {
                    userId,
                    amount: cost,
                    type: 'refund',
        ...
        recordsToCreate.push(prisma.notification.create({
            data: {
                userId,
