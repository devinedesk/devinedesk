...
const translations = {
  en: {
    ...
    'settings.Local APIKeyLabel': 'API Key',
    'settings.keyPlaceholder': 'Enter your Platform API key...',
    'settings.keyNote': 'Your API key is stored locally and never sent anywhere except locally.',
    'settings.invalidKey': 'Please enter a valid API key.',

    // Auth Modal
    'auth.title': 'API Key Required',
    'auth.subtitle':
      'Create a Platform access key, then paste the key value here to start creating high-aesthetic images.',
    'auth.keyLabel': 'Platform Access Key',
    'auth.keyPlaceholder': 'Paste your access key value...',
    'auth.keyNote':
      'Do not enter the key name or label; paste the generated key value from Platform.',
    'auth.initBtn': 'Initialize Studio',
    'auth.createKey': 'Create or copy a Platform access key →',
    ...
    'image.negPromptPlaceholder':
      'What to exclude from the image (e.g., blurry, distorted, watermark)',
    ...
    'agents.webOnly': 'Available in the web app at devinedesk.com',

    // Workflow Studio
    'workflows.title': 'Workflow Studio',
    'workflows.webOnly': 'Available in the web app at devinedesk.com',
    ...
    'localModels.deleteConfirm': (name) =>
      `Delete "${name}"? You'll need to re-download it to use it again.`,
    ...
    'mcp.title': 'MCP & CLI',
    'mcp.subtitle':
      'Use devinedesk from your terminal, your IDE, or any MCP-compatible assistant. Generate cinematic images, videos, and audio across 100+ models — without leaving your workflow.',
  ...
  zh: {
    ...
    'settings.Local APIKeyLabel': 'Platform API 密钥',
    'settings.keyPlaceholder': '输入您的 Platform API 密钥...',
    'settings.keyNote': '您的 API 密钥仅存储在本地，除本地外不会发送到任何地方。',
    'settings.invalidKey': '请输入有效的 API 密钥。',

    // Auth Modal
    'auth.title': '需要 Platform API 密钥',
    'auth.subtitle': '创建一个 Platform 访问密钥，然后将密钥值粘贴到这里开始创建高质量图像。',
    'auth.keyLabel': 'Platform 访问密钥',
    'auth.keyPlaceholder': '粘贴您的访问密钥值...',
    'auth.keyNote': '请不要输入密钥名称或标签；粘贴从 Platform 生成的密钥值。',
    'auth.initBtn': '初始化工作室',
    'auth.createKey': '创建或复制 Platform 访问密钥 →',
    ...
    // Agent Studio
    'agents.title': '智能体工作室',
    'agents.webOnly': '在网页应用 devinedesk.com 上可用',

    // Workflow Studio
    'workflows.title': '工作流工作室',
    'workflows.webOnly': '在网页应用 devinedesk.com 上可用',

    // Local Model Manager
    'localModels.title': '本地模型',
    'localModels.webOnly':
      '本地模型推理仅在桌面应用（Electron 构建版）中可用。使用 npm run electron:build 进行构建。',
    ...
    'mcp.title': 'MCP & CLI',
    'mcp.subtitle':
      '在终端、IDE 或任何兼容 MCP 的助手中使用 devinedesk。跨 100+ 模型生成电影级图像、视频和音频 — 无需离开您的工作流。',
    'mcp.quickStart': '快速开始',
  },
};
