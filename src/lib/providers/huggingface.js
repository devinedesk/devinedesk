export const huggingfaceAdapter = {
  getKey: (params) => {
    ...
    if (!key)
      throw new Error('Hugging Face Token missing. Please set it in Settings or Environment.');
    return key;
  },
  ...
  generate3D: async function (params) {
    const key = this.getKey(params);
    const endpointId = params.model || 'stabilityai/stable-fast-3d';
    const url = `https://api-inference.huggingface.co/models/${endpointId}`;

    // Just simulating the API structure since HF 3D endpoints often vary
    const response = await fetch(url, {
      method: 'POST',
      headers: {
        Authorization: `Bearer ${key}`,
      ...
      body: JSON.stringify({ inputs: params.image_url || params.prompt }),
    });
