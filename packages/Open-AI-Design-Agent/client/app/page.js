                      ...
                      {attachments.map((att, i) => (
                        ...
                        >
                          <div className="relative w-5 h-5 rounded overflow-hidden">
                            {att.kind === "image" ? <Image src={att.url} alt="Attachment" fill className="object-cover" /> : <FiTerminal size={10} />}
                          </div>
                      ...
                      {input.match(/@asset_\d+/g)?.map(match => {
                        ...
                        return (
                          <div 
                            ...
                            onMouseLeave={() => setHoveredAsset(null)}
                          >
                            <div className="relative w-5 h-5 rounded overflow-hidden bg-primary/10 flex items-center justify-center text-primary">
                              {asset.kind === "image" ? <Image src={asset.url} alt="Asset" fill className="object-cover" /> : <RiSparklingLine size={10} />}
                            </div>
                  ...
                  {hoveredAsset && (
                    <div className="absolute bottom-full left-0 mb-10 w-72 aspect-square bg-bg-card border border-divider rounded-md shadow-[0_32px_64px_-12px_rgba(0,0,0,0.2)] overflow-hidden z-[110] animate-in fade-in zoom-in-95 duration-200 pointer-events-none">
                      {hoveredAsset.kind === "image" ? (
                        <Image src={hoveredAsset.url} alt="Preview" fill className="object-cover" />
                ...
                <div className="flex items-center gap-2">
                  {attachments.length > 0 && (
                    <div className="flex items-center -space-x-2 mr-2">
                      {attachments.map((a, i) => (
                        <div key={i} className="relative w-6 h-6 rounded-full border-2 border-bg-card bg-bg-page overflow-hidden shadow-sm">
                          {a.kind === "image" ? <Image src={a.url} alt="Attachment" fill className="object-cover" /> : <div className="w-full h-full flex items-center justify-center bg-black"><FiTerminal size={10} className="text-white" /></div>}
                        </div>
                      ))}
