...
export function GenerationCopyButtons({ prompt, imageUrl, onCopyError }) {
    ...
    <>
      {prompt && (
        <button
          ...
          className={`flex h-8 w-8 items-center justify-center rounded-full border border-white/10 bg-black/60 backdrop-blur-md transition-all hover:bg-primary hover:text-black ${
            copiedKind === 'text' ? 'text-primary' : 'text-white'
          }`}
        >
          {copiedKind === 'text' ? <CopiedIcon /> : <CopyContentIcon kind="text" size={17} />}
        </button>
      )}
      {imageUrl && (
        <button
          ...
          className={`flex h-8 w-8 items-center justify-center rounded-full border border-white/10 bg-black/60 backdrop-blur-md transition-all hover:bg-primary hover:text-black ${
            copiedKind === 'image' ? 'text-primary' : 'text-white'
          }`}
        >
          {copiedKind === 'image' ? <CopiedIcon /> : <CopyContentIcon kind="image" size={17} />}
        </button>
      )}
    </>
  );
...
export default function MobileGenerationActions({ actions = [], prompt, imageUrl, onCopyError }) {
      ...
      ? {
          ...
          onSelect: async () => {
            try {
              ...
              onCopyError?.('Could not copy the image. Image copy requires HTTPS or localhost.');
            }
          },
        }
      : null,
