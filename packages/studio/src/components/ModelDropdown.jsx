...
export function ModelDropdown({ models, toolsModels = [], selectedModel, onSelect, onClose, useLocalModel = false, modelType = "Models", toolsType = "Tools" }) {
  ...
  const getProviderStyle = (provider) => {
    switch (provider) {
      case "local": return { text: "L", bg: "bg-green-500/10 text-green-400 border-green-500/25" };
    ...
    >
      <div className="flex items-center gap-3.5">
        {PROVIDER_LOGOS[m.provider] ? (
          ...
          <div className={`w-9 h-9 ${getIconColor(m, isTool)} border rounded-xl flex items-center justify-center font-black text-xs shadow-inner uppercase`}>
            {m.name.charAt(0)}
          </div>
        )}
        <div className="flex flex-col gap-0.5 min-w-0">
          ...
          {isTool && m.imageField ? (
            ...
            selectedProvider === "all" && m.provider_name && (
              <span className="text-[9px] text-white/60">{m.provider_name}</span>
            )
          )}
        </div>
      </div>
      ...
      <div className="flex flex-col gap-2.5 items-center pr-2 border-r border-white/5 shrink-0 select-none overflow-y-auto custom-scrollbar w-14 pt-0.5">
        <button
          ...
          className={`w-8 h-8 rounded-full flex items-center justify-center border transition-all flex-shrink-0 cursor-pointer ${
            selectedProvider === "all" ? "bg-white/10 text-yellow-400 border-yellow-500/30 shadow-md scale-105" : "bg-white/[0.02] text-white/50 border-white/[0.03] hover:bg-white/5 hover:text-white"
          }`}
          title="All Providers"
        >
          ...
          return (
            <button
              ...
              className={`w-8 h-8 flex-shrink-0 rounded-full flex items-center justify-center font-black text-[10px] border transition-all flex-shrink-0 cursor-pointer overflow-hidden ${
                isSelected ? `${style.bg} border-white/25 scale-105 shadow-md` : "bg-white/[0.02] text-white/60 border-white/[0.02] hover:bg-white/5 hover:text-white/80"
              }`}
              title={p.name}
            >
              {PROVIDER_LOGOS[p.id] ? (
                <img src={PROVIDER_LOGOS[p.id]} alt={p.name} className={`w-full h-full rounded-full object-contain ${invertLogos.includes(p.id) ? "invert" : ""}`} />
              ) : style.text}
            </button>
          );
        })}
          ...
          {filteredMain.length === 0 && filteredTools.length === 0 ? (
            ...
            <>
              ...
              {filteredTools.length > 0 && (
                <>
                  <div className="text-xs font-bold text-orange-400/70 px-3 py-2 mt-1 border-t border-white/5">{toolsType}</div>
