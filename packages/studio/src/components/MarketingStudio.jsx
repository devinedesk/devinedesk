...
import { uploadFile, generateMarketingStudioAd } from '../apiClient.js';
    ...
    <div className="relative group/slot flex items-center">
      ...
      >
        ...
        {progress > 0 && progress < 100 ? (
          ...
          <div className="text-white/60 group-hover:text-primary transition-colors">{icon}</div>
        )}
...
function Dropdown({
  ...
  return (
    <PromptPopover ref={ref} className="w-[420px] max-w-[calc(100vw-2rem)]">
      ...
      for (const file of toUpload) {
        try {
          const url = await uploadFile(apiKey, file, (pct) =>
            setUploadProgress((p) => ({ ...p, additional: pct }))
          );
        ...
        {history.length > 0 ? (
          <StudioGallery
            ...
            onDownload={(entry, idx) =>
              downloadFile(entry.url, `marketing-ad-${entry.id || idx}.mp4`)
            }
            onDelete={(entry, idx) => {
              if (!historyItems) {
            ...
            studioName="Marketing Studio"
          />
        ) : (
          <EmptyStateHero
            selectedModelName="MARKETING STUDIO"
            title="START CREATING WITH"
            subtitle="Describe your scene, upload your product, and watch high-converting AI video ads come to life."
          />
        )}
      </div>
            ...
            {['ratio', 'res', 'duration'].map((key) => (
              <div key={key} className="relative">
                ...
                <SimpleDropdown
                  isOpen={dropdown === key}
                  title={
                    key === 'ratio' ? 'Aspect Ratio' : key === 'res' ? 'Resolution' : 'Duration'
                  }
                  options={OPTIONS[key]}
                  selected={params[key]}
          ...
          {previewAvatar.id !== 'custom' && (
            <button
              ...
              onClick={(e) => {
                ...
                if (currentIndex !== -1) {
                  const prevAvatar =
                    ASSETS.avatar[(currentIndex - 1 + ASSETS.avatar.length) % ASSETS.avatar.length];
            ...
            {previewAvatar.id !== 'custom' && (
              <div
                onClick={(e) => {
                  ...
                  if (currentIndex !== -1) {
                    const prevAvatar =
                      ASSETS.avatar[
                        (currentIndex - 1 + ASSETS.avatar.length) % ASSETS.avatar.length
                      ];
                    setSlideDirection('prev');
                    setPreviewAvatar(prevAvatar);
                  }
              ...
              >
                <img
                  src={
                    ASSETS.avatar[
                      (ASSETS.avatar.findIndex((a) => a.id === previewAvatar.id) -
                        1 +
            ...
            {previewAvatar.id !== 'custom' && (
              ...
              >
                <img
                  src={
                    ASSETS.avatar[
                      (ASSETS.avatar.findIndex((a) => a.id === previewAvatar.id) + 1) %
                        ASSETS.avatar.length
                    ].url
                  }
                  alt="Next Avatar"
