...
function notifyAuthRequired(status, detail) {
  ...
  window.dispatchEvent(
    new CustomEvent('platform:auth-required', { detail: { status, message: detail } })
  );
}
...
async function pollForQueueJob(jobId, maxAttempts = 300, interval = 2000) {
  ...
  for (let attempt = 1; attempt <= maxAttempts; attempt++) {
    ...
    try {
      ...
      if (status === 'failed' || status === 'error')
        throw new Error(`Queue job failed: ${data.error || JSON.stringify(data)}`);
    } catch (e) {
...
export async function pollAgentChatResult(
  apiKey,
  requestId,
  { maxAttempts = 150, interval = 2000 } = {}
) {
...
export async function updateWorkflowName(apiKey, workflowId, name) {
  return apiFetch(`/workflow/update-name/${workflowId}`, {
    apiKey,
    method: 'POST',
    body: { name },
  });
...
export async function executeWorkflow(apiKey, workflowId, inputs) {
  const submitData = await apiFetch(`/workflow/${workflowId}/api-execute`, {
    apiKey,
    method: 'POST',
    body: { inputs },
  });
...
export async function runSingleNode(apiKey, workflowId, nodeId, payload) {
  return apiFetch(`/workflow/${workflowId}/node/${nodeId}/run`, {
    apiKey,
    method: 'POST',
    body: payload,
  });
