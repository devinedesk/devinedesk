...
const inputHandles = ['textInput', 'textInput2', 'textInput3', 'textInput4'];

const outputHandles = ['textOutput'];
...
const TextGeneration = ({ id, data, selected }) => {
  ...
  const properties =
    nodeSchemas?.categories?.text?.models?.[selectedModel.id]?.input_schema?.schemas?.input_data
      ?.properties;
  ...
  const hasSystemPrompt =
    properties && 'system_prompt' in properties && !data.selectedModel?.id?.includes('passthrough');
