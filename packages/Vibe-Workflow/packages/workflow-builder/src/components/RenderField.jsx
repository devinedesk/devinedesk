...
const RenderField = ({
  ...
  const handleFileUpload = (field, fieldSchema, e) => {
    ...
    const cloudName =
      window.__CLOUDINARY_CLOUD_NAME__ || localStorage.getItem('cloudinary_cloud_name');
    const uploadPreset =
      window.__CLOUDINARY_UPLOAD_PRESET__ || localStorage.getItem('cloudinary_upload_preset');
      ...
      <div key={fieldName} className="flex flex-col gap-1">
        ...
        <div className="grid grid-cols-3 gap-2">
          {imageList.map((url, idx) => (
            <div
              key={idx}
              className={`flex items-center gap-2 relative group overflow-hidden ${['audios_list', 'audio_files'].includes(meta.field) ? 'col-span-full' : ''}`}
            >
      ...
      <div key={fieldName} className="flex flex-col gap-2">
        ...
        <div className="flex items-center gap-2">
          ...
          >
            ...
            >
              <span
                className={`h-[12px] w-[12px] rounded-full bg-white duration-200 shadow-sm ${!!formValues[fieldName] && 'translate-x-[16px]'}`}
              ></span>
            </span>
          </label>
