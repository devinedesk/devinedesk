...
const outputHandles = ['videoOutput'];
...
const VideoGeneration = ({ id, data, selected }) => {
  ...
  const [selectedModel, setSelectedModel] = useState(
    data.selectedModel || models[1] || models[0] || {}
  );
  ...
  const properties =
    nodeSchemas?.categories?.video?.models?.[selectedModel.id]?.input_schema?.schemas?.input_data
      ?.properties;
  ...
  const hasPrompt =
    properties && 'prompt' in properties && !data.selectedModel?.id.includes('passthrough');
  const hasImagesList =
    properties && 'images_list' in properties && !data.selectedModel?.id.includes('passthrough');
  const hasVideosList =
    properties &&
    ('videos_list' in properties || 'video_files' in properties) &&
    !data.selectedModel?.id.includes('passthrough');
  const hasLastImage =
    properties && 'last_image' in properties && !data.selectedModel?.id.includes('passthrough');
  const hasImageUrl =
    properties && 'image_url' in properties && !data.selectedModel?.id.includes('passthrough');
  const hasVideoUrl =
    properties && 'video_url' in properties && !data.selectedModel?.id.includes('passthrough');
  const hasAudioUrl =
    properties && 'audio_url' in properties && !data.selectedModel?.id.includes('passthrough');
  const hasAudiosList =
    properties &&
    ('audios_list' in properties || 'audio_files' in properties) &&
    !data.selectedModel?.id.includes('passthrough');
  ...
  return (
    ...
    >
      ...
      <div className="flex flex-col">
        <div className="flex items-center justify-between bg-gradient-to-r from-panel-bg to-card-bg rounded-t-2xl border-b border-zinc-800 p-3">
          <div className="flex items-center gap-2.5">
            <div
              className={`p-1.5 rounded-lg ${selected ? 'bg-orange-600 text-white' : 'bg-zinc-800 text-zinc-400'} transition-colors`}
            >
        ...
        <div className="flex items-center flex-grow justify-center w-full h-full rounded transition-all duration-500">
          {data.isLoading ? (
            ...
            <div className="h-full w-full relative">
              <VideoPlayer key={currentOutput} src={currentOutput} accentColor="#f97316" />
      ...
      {hasPrompt && (
        <p
          className={`absolute -left-8 top-[70px] text-xs text-cyan-400 transition-opacity duration-200 ${
      ...
      <p
        className={`absolute -right-10 top-[100px] text-xs text-orange-500 transition-opacity duration-200 ${
          data.activeHandleColor === 'orange' ? 'opacity-100' : 'opacity-0 group-hover:opacity-100'
        }`}
      >
        Video
      </p>
