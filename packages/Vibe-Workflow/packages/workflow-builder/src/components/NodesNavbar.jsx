...
import { TbArrowMerge, TbWorldWww } from 'react-icons/tb';
...
const NodesNavbar = ({ addNode, apiNodeModels, filterNodeTypes = null, nodeSchemas = {} }) => {
  ...
  const getNodeTypeFromSubmenuId = (id) => {
    ...
    if (id === 'concat' || id === 'text-utils' || id === 'utilities')
      return ['concatNode', 'vidConcatNode', 'httpNode'];
  ...
  const getCategorizedModels = () => {
    ...
    const generateImageModels = imageModels.filter(
      (m) =>
        m?.id &&
        !isPassthrough(m) &&
        !m.id.includes('edit') &&
        !m.id.includes('reference') &&
        !m.id.includes('image-to-image')
    );
    const editImageModels = imageModels.filter(
      (m) =>
        m?.id &&
        !isPassthrough(m) &&
        (m.id.includes('edit') || m.id.includes('reference') || m.id.includes('image-to-image'))
    );
    const upscaleImageModels = imageModels.filter(
      (m) => m?.id && !isPassthrough(m) && m.id.includes('upscale')
    );
    const generateVideoModels = videoModels.filter(
      (m) => m?.id && !isPassthrough(m) && !m.id.includes('edit')
    );
    const editVideoModels = videoModels.filter(
      (m) => m?.id && !isPassthrough(m) && m.id.includes('edit')
    );
  ...
  const menuStructure = [
    ...
    },
    {
      ...
      items: [
        ...
        {
          label: 'Utilities',
          icon: <TbArrowMerge className="rotate-90" />,
          hasSubmenu: true,
          id: 'utilities',
        },
      ],
  ...
  const getSubmenuItems = (id) => {
    switch (id) {
      case 'inputs':
        return categorizedModels.inputs.map((m) => ({ label: m.name, model: m, type: m.type }));
      case 'text-utils':
      case 'utilities':
        return [
          ...categorizedModels.utilities.map((m) => ({
            label: m.name,
            model: m,
          ...
          {
            label: 'HTTP Request',
            model: { id: 'http-request', name: 'HTTP Request' },
            type: 'httpNode',
          },
        ];
      case 'generate-image':
        return categorizedModels.generateImage.map((m) => ({
          label: m.name,
          model: m,
          type: 'imageNode',
        }));
      case 'edit-image':
        return categorizedModels.editImage.map((m) => ({
          label: m.name,
          model: m,
          type: 'imageNode',
        }));
      case 'upscale-image':
        return categorizedModels.upscaleImage.map((m) => ({
          label: m.name,
          model: m,
          type: 'imageNode',
        }));
      case 'text-llms':
        return categorizedModels.text.map((m) => ({ label: m.name, model: m, type: 'textNode' }));
      case 'generate-video':
        return categorizedModels.generateVideo.map((m) => ({
          label: m.name,
          model: m,
          type: 'videoNode',
        }));
      case 'edit-video':
        return categorizedModels.editVideo.map((m) => ({
          label: m.name,
          model: m,
          type: 'videoNode',
        }));
      case 'generate-audio':
        return categorizedModels.audio.map((m) => ({ label: m.name, model: m, type: 'audioNode' }));
      case 'api-models':
        return categorizedModels.api.map((m) => ({ label: m.name, model: m, type: 'apiNode' }));
      default:
        return [];
    }
  };
  ...
  const renderSearchResults = () => {
    ...
    const allModels = [
      ...
      { id: 'http-request', name: 'HTTP Request', type: 'httpNode' },
      ...apiNodeModels.map((m) => ({ ...m, type: 'apiNode' })),
    ];

    const filtered = allModels.filter(
      (m) => m && m.name && m.name.toLowerCase().includes(searchQuery.toLowerCase())
    );
        ...
        {filtered.length > 0 ? (
          filtered.map((item, idx) => (
            ...
            >
              ...
              {item.type === 'vidConcatNode' && <TfiVideoClapper />}
              {item.type === 'httpNode' && <TbWorldWww />}
          ...
          <div className="flex flex-col gap-3 overflow-y-auto custom-scrollbar min-h-0">
            {filteredMenuStructure.map((section, idx) => (
              <div key={idx} className="flex flex-col gap-1">
                <h3 className="text-[10px] text-gray-500 text-left px-2 font-medium sticky top-0 bg-[#151618] z-10">
                  {section.label}
                </h3>
