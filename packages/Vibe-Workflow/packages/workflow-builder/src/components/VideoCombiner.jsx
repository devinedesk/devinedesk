...
const VideoCombiner = ({ id, data, selected }) => {
  ...
  const [formValues, setFormValues] = useState(
    data.formValues || { videos_list: [], aspect_ratio: 'auto' }
  );
  ...
  const properties =
    nodeSchemas?.categories?.utility?.models?.[selectedModel.id]?.input_schema?.schemas?.input_data
      ?.properties;
  ...
  const handleRunSingleNode = async () => {
    ...
    try {
      ...
      const modelSchema =
        nodeSchemas?.categories?.utility?.models[selectedModel.id]?.input_schema?.schemas
          ?.input_data;
  ...
  useEffect(() => {
    const timeout = setTimeout(() => {
      const validHandles = [hasVideosList && 'videoInput7'].filter(Boolean);
  ...
  return (
    ...
    >
      ...
      {hasVideosList && (
        <p
          className={`absolute -left-10 top-[100px] text-xs text-orange-500 transition-opacity duration-200 ${data.activeHandleColor === 'orange' ? 'opacity-100' : 'opacity-0 group-hover:opacity-100'}`}
        >
          Videos
        </p>
      )}
      ...
      />
      <p
        className={`absolute -right-10 top-[100px] text-xs text-orange-500 transition-opacity duration-200 ${data.activeHandleColor === 'orange' ? 'opacity-100' : 'opacity-0 group-hover:opacity-100'}`}
      >
        Video
      </p>
    </div>
