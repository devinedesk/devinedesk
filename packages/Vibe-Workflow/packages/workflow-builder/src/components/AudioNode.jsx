...
const inputHandles = ['audioInput', 'audioInput2', 'audioInput3', 'audioInput4'];

const outputHandles = ['audioOutput'];
...
const AudioGeneration = ({ id, data, selected }) => {
  ...
  const properties =
    nodeSchemas?.categories?.audio?.models?.[selectedModel.id]?.input_schema?.schemas?.input_data
      ?.properties;
  ...
  return (
    ...
    >
      ...
      <div className="flex flex-col">
        <div className="flex items-center justify-between bg-gradient-to-r from-panel-bg to-card-bg rounded-t-2xl border-b border-zinc-800 py-2 px-3">
          <div className="flex items-center gap-2.5">
            <div
              className={`p-1.5 rounded-lg ${selected ? 'bg-yellow-500 text-black' : 'bg-zinc-800 text-zinc-400'} transition-colors`}
            >
      ...
      {hasPrompt && (
        <p
          className={`absolute -left-8 top-[100px] text-xs text-cyan-400 transition-opacity duration-200 ${
      ...
      <p
        className={`absolute -right-10 top-[100px] text-xs text-yellow-500 transition-opacity duration-200 ${
          data.activeHandleColor === 'yellow' ? 'opacity-100' : 'opacity-0 group-hover:opacity-100'
        }`}
      >
        Audio
      </p>
