...
const inputHandles = ['imageInput', 'imageInput2', 'imageInput3'];

const outputHandles = ['imageOutput'];
...
const ImageGeneration = ({ id, data, selected }) => {
  ...
  const properties =
    nodeSchemas?.categories?.image?.models?.[selectedModel.id]?.input_schema?.schemas?.input_data
      ?.properties;
      ...
      <div className="flex flex-col">
        <div className="flex items-center justify-between bg-gradient-to-r from-panel-bg to-card-bg rounded-t-2xl border-b border-zinc-800 p-3">
          <div className="flex items-center gap-2.5">
            <div
              className={`p-1.5 rounded-lg ${selected ? 'bg-emerald-600 text-white' : 'bg-zinc-800 text-zinc-400'} transition-colors`}
            >
              ...
              <div className="absolute inset-x-0 bottom-0 p-4 bg-gradient-to-t from-black/80 via-black/40 to-transparent opacity-0 group-hover/image:opacity-100 transition-opacity duration-300 pointer-events-none rounded-b-xl flex flex-col justify-end">
                <div className="flex items-center justify-between">
                  <div className="flex flex-col gap-0.5">
                    <span className="text-[10px] text-white/50 uppercase tracking-tighter font-semibold">
                      Dimensions
                    </span>
